...
struct CompareDistance
{
	...
	{
		return get_world_space_coords(a.cached_head_bone, a.cached_component_to_world).distance(cache::local_camera.location) < get_world_space_coords(b.cached_head_bone, b.cached_component_to_world).distance(cache::local_camera.location);
	}
};
	...
	while (true) {
		{
			...
			for (int i = 0; i < player_count; i++) {
				uintptr_t player_state = mem.Read<uintptr_t>(player_array_cursor + i * 0x8);
				...
				cache::mesh_info.push_back({ head_bone, head_transform, component_to_world, component_to_world_transform });
			}
